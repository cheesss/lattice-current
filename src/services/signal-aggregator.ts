...
class SignalAggregator {
  ...
  ingestGlintSignals(signals: GlintCountrySignal[]): void {
    this.clearSignalType('glint_signal');
    for (const signal of signals) {
      if (!signal || signal.signalCount <= 0) continue;
      const sourceDiversity = Object.values(signal.sourceCounts).filter((count) => count > 0).length;
      const severity: 'low' | 'medium' | 'high' =
        signal.severityCounts.critical > 0 || signal.severityCounts.high >= 3
          ? 'high'
          : signal.severityCounts.high > 0 || signal.severityCounts.medium >= 3
            ? 'medium'
            : 'low';

      this.signals.push({
        type: 'glint_signal',
        country: normalizeCountryCode(signal.code),
        countryName: signal.countryName || getCountryName(signal.code),
        lat: Number.isFinite(signal.lat) ? signal.lat : 0,
        lon: Number.isFinite(signal.lon) ? signal.lon : 0,
        severity,
        title: `Glint signal: ${signal.signalCount} events, ${sourceDiversity} source types`,
        timestamp: signal.lastTimestamp ?? new Date(),
      });
    }
    this.pruneOld();
  }
    ...
    for (const [_regionId, def] of Object.entries(REGION_DEFINITIONS)) {
      ...
      if (allTypes.size >= 2) {
        const typeLabels: Record<SignalType, string> = {
          ...
          glint_signal: 'glint intelligence',
