...
export async function fetchHapiSummary(): Promise<Map<string, HapiConflictSummary>> {
  ...
  const fetchFallbackResults = async (): Promise<GetHumanitarianSummaryBatchResponse> => {
  ...
  const resp = await hapiBatchBreaker.execute(async () => {
    try {
      ...
      if (!batch || typeof batch !== 'object' || !batch.results || typeof batch.results !== 'object') {
        return fetchFallbackResults();
      }
      return batch;
    } catch (err: unknown) {
...
export async function fetchUcdpEvents(hydrated?: ListUcdpEventsResponse): Promise<UcdpEventsResponse> {
  const hydratedEvents = hydrated ?? (getHydratedData('ucdpEvents') as ListUcdpEventsResponse | undefined);
  if (hydratedEvents?.events?.length) {
    const events = hydratedEvents.events.map(toUcdpGeoEvent);
