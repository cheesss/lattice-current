...
export function getGlobeRenderScale(): GlobeRenderScale {
  ...
  // Default to eco profile to keep desktop GPU usage predictable.
  return '1';
}
...
export function resolvePerformanceProfile(scale: GlobeRenderScale): GlobePerformanceProfile {
  const isEco = scale === '1';
  const isAuto = scale === 'auto';
  return {
    disablePulseAnimations: isEco || isAuto,
    disableDashAnimations: isEco || isAuto,
