...
const COUNTRY_GEOJSON_REMOTE_FALLBACK_URL = 'https://maps.worldmonitor.app/countries.geojson';
...
const COUNTRY_OVERRIDES_REMOTE_FALLBACK_URL = 'https://maps.worldmonitor.app/country-boundary-overrides.geojson';
...
async function ensureLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    ...
    try {
      ...
      const geoJsonUrls = [COUNTRY_GEOJSON_URL, COUNTRY_GEOJSON_REMOTE_FALLBACK_URL];
      ...
      if (!response) {
        throw lastError instanceof Error ? lastError : new Error('countries.geojson unavailable');
      }
      ...
      try {
        const overrideUrls = [COUNTRY_OVERRIDES_URL, COUNTRY_OVERRIDES_REMOTE_FALLBACK_URL];
        for (const url of overrideUrls) {
          try {
            const overrideResp = await fetch(url, {
            ...
            // Overrides optional; try next source.
          }
        }
      } catch {
