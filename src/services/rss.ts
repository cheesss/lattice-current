...
const PERSISTENT_FEED_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours max disk fallback age for news feeds
...
async function readPersistentFeed(key: string): Promise<NewsItem[] | null> {
  ...
  if (entry.updatedAt && Date.now() - entry.updatedAt > PERSISTENT_FEED_TTL_MS) {
    void deletePersistentCache(key).catch(() => { });
    return null;
  }
...
export async function fetchFeed(feed: Feed): Promise<NewsItem[]> {
  ...
  try {
    ...
    url = await resolveFeedUrlForFetch(feed.name, currentLang, url);
    selectedUrl = url;
    ...
    void onFeedFetchSuccess(feed.name, currentLang, selectedUrl || url).catch(() => { });
