...
export const RUNTIME_FEATURES: RuntimeFeatureDefinition[] = [
  {
    ...
    fallback: 'Falls back to Groq, then OpenAI, then Codex Login, then OpenRouter, then local browser model.',
  },
  {
    id: 'aiGroq',
    ...
    fallback: 'Falls back to OpenAI, then Codex Login, then OpenRouter, then local browser model.',
  },
  {
    id: 'aiOpenAI',
    name: 'OpenAI summarization',
    description: 'Codex-compatible OpenAI chat completions provider for AI summary generation.',
    requiredSecrets: ['OPENAI_API_KEY'],
    fallback: 'Falls back to Codex Login, then OpenRouter, then local browser model.',
  },
  {
    id: 'aiCodexLogin',
    name: 'Codex login summarization',
    description: 'Uses local Codex CLI authenticated via ChatGPT login to generate summaries.',
    requiredSecrets: [],
  ...
  {
    id: 'openbbSignals',
    name: 'OpenBB cross-asset intelligence',
    description: 'Cross-asset tape and impact/exposure panels backed by OpenBB API.',
    requiredSecrets: ['OPENBB_API_URL'],
    desktopRequiredSecrets: [],
    fallback: 'Panels fall back to local market feeds and reduced analytics.',
  },
];
...
export function validateSecret(key: RuntimeSecretKey, value: string): { valid: boolean; hint?: string } {
  ...
  if (key === 'OPENBB_API_KEY') {
    if (trimmed.length < 8) return { valid: false, hint: 'API key must be at least 8 characters' };
    return { valid: true };
  }
