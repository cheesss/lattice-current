...
export function debugInjectTestEvents(): void {
  const now = Date.now();
  ingestGeoEvent(32.1, 35.2, 'protest', new Date(now - 15 * 60 * 1000));
  ingestGeoEvent(32.2, 35.3, 'military_flight', new Date(now - 10 * 60 * 1000));
  ingestGeoEvent(32.0, 35.1, 'military_vessel', new Date(now - 5 * 60 * 1000));
}
