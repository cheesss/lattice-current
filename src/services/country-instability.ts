import type { SocialUnrestEvent, MilitaryFlight, MilitaryVessel, ClusteredEvent, InternetOutage, AisDisruptionEvent, AisDensityZone } from '@/types';
...
import { dataFreshness, type DataSourceId } from './data-freshness';
...
import type { GlintCountrySignal } from './glint-intelligence';
...
interface CountryData {
  ...
  maritimeDisruptions: number;
  maritimeDensityStress: number;
  ...
  glint: CountryGlintSnapshot | null;
}

interface CountryGlintSnapshot {
  signalCount: number;
  critical: number;
  high: number;
  medium: number;
...
const recentMaritimeIngest = new Map<string, number>();
const MARITIME_INGEST_TTL_MS = 3 * 60 * 60 * 1000;
...
function initCountryData(): CountryData {
  return {
    ...
    militaryFlights: [],
    militaryVessels: [],
    maritimeDisruptions: 0,
    maritimeDensityStress: 0,
    newsEvents: [],
    outages: [],
...
export function hasIntelligenceSignalsLoaded(): boolean {
  if (countryDataMap.size === 0) return false;
  for (const data of countryDataMap.values()) {
    if (
      ...
      || data.maritimeDisruptions > 0
      || data.maritimeDensityStress > 0
...
export function ingestGlintForCII(signals: GlintCountrySignal[]): void {
  for (const data of countryDataMap.values()) {
    data.glint = null;
  }
  ...
  for (const signal of signals) {
    processedCount++;
    const code = ensureISO2(signal.code);
    if (!code) {
      unmappedCount++;
      continue;
    }
    ...
    if (!countryDataMap.has(code)) countryDataMap.set(code, initCountryData());
    const target = countryDataMap.get(code)!;
    const sourceDiversity = Object.values(signal.sourceCounts).filter((count) => count > 0).length;

    target.glint = {
      signalCount: signal.signalCount,
      critical: signal.severityCounts.critical,
      high: signal.severityCounts.high,
      medium: signal.severityCounts.medium,
      low: signal.severityCounts.low,
      info: signal.severityCounts.info,
      sourceDiversity,
      geopoliticalHits: signal.geopoliticalHits,
      economicHits: signal.economicHits,
      securityHits: signal.securityHits,
      marketShock: signal.marketShock,
      marketVolume: signal.marketVolume,
    };

    if (Number.isFinite(signal.lat) && Number.isFinite(signal.lon)) {
      const hotspotWeight = 1
        + signal.severityCounts.critical * 1.5
        + signal.severityCounts.high
        + signal.securityHits * 0.4;
      trackHotspotActivity(signal.lat, signal.lon, hotspotWeight);
    }
  }
}
...
function shouldIngestMaritimeEvent(key: string): boolean {
  const now = Date.now();
  const seenAt = recentMaritimeIngest.get(key);
  if (seenAt && now - seenAt < MARITIME_INGEST_TTL_MS) return false;
  recentMaritimeIngest.set(key, now);
  if (recentMaritimeIngest.size > 5000) {
    const cutoff = now - MARITIME_INGEST_TTL_MS;
    for (const [k, ts] of recentMaritimeIngest) {
      if (ts < cutoff) recentMaritimeIngest.delete(k);
    }
  }
  return true;
}

function resolveCountryFromMaritimeEvent(lat: number, lon: number, text: string): string | null {
  const byCoord = getCountryFromLocation(lat, lon);
  if (byCoord) return byCoord;
  const fromName = matchCountryNamesInText(text.toLowerCase());
  if (fromName.length > 0) return fromName[0]!;
  return null;
}

export function ingestAisForCII(disruptions: AisDisruptionEvent[], density: AisDensityZone[]): void {
  for (const event of disruptions) {
    const dedupeKey = `d:${event.id || `${event.name}:${event.lat.toFixed(2)}:${event.lon.toFixed(2)}`}`;
    if (!shouldIngestMaritimeEvent(dedupeKey)) continue;
    processedCount++;
    const text = `${event.name} ${event.description || ''} ${event.region || ''}`;
    const code = resolveCountryFromMaritimeEvent(event.lat, event.lon, text);
    if (!code) { unmappedCount++; continue; }
    if (!countryDataMap.has(code)) countryDataMap.set(code, initCountryData());

    const severityWeight = event.severity === 'high' ? 3 : event.severity === 'elevated' ? 2 : 1;
    const changeWeight = Math.max(0.5, Math.min(3, Math.abs(event.changePct) / 30));
    const darkShipWeight = event.darkShips ? Math.min(2, event.darkShips / 8) : 0;
    countryDataMap.get(code)!.maritimeDisruptions += severityWeight + changeWeight + darkShipWeight;
    trackHotspotActivity(event.lat, event.lon, 1.8 + severityWeight * 0.4);
  }

  for (const zone of density) {
    const dedupeKey = `z:${zone.id || `${zone.name}:${zone.lat.toFixed(2)}:${zone.lon.toFixed(2)}`}`;
    if (!shouldIngestMaritimeEvent(dedupeKey)) continue;
    processedCount++;
    const text = `${zone.name} ${zone.note || ''}`;
    const code = resolveCountryFromMaritimeEvent(zone.lat, zone.lon, text);
    if (!code) { unmappedCount++; continue; }
    if (!countryDataMap.has(code)) countryDataMap.set(code, initCountryData());
    const intensityWeight = Math.max(0.2, Math.min(3, zone.intensity / 35));
    const deltaWeight = Math.max(0.2, Math.min(3, Math.abs(zone.deltaPct) / 18));
    countryDataMap.get(code)!.maritimeDensityStress += intensityWeight + deltaWeight;
    trackHotspotActivity(zone.lat, zone.lon, 0.8 + intensityWeight * 0.5);
  }
}
...
function calcInformationScore(data: CountryData, countryCode: string): number {
  ...
  const glintBoost = data.glint
    ? Math.min(
      35,
      data.glint.signalCount * 2
      + data.glint.sourceDiversity * 3
      + data.glint.high * 1.5
      + data.glint.critical * 2.5
      + data.glint.economicHits * 1.2
    )
    : 0;
...
const COMPONENT_SOURCE_MAP: Record<keyof ComponentScores, DataSourceId[]> = {
  unrest: ['acled', 'outages', 'glint'],
  conflict: ['acled_conflict', 'ucdp', 'hapi', 'glint'],
  security: ['opensky', 'ais', 'portwatch', 'glint'],
  information: ['rss', 'gdelt', 'gdelt_doc', 'glint', 'security_advisories'],
};

function isSourceUsable(id: DataSourceId): boolean {
  const source = dataFreshness.getSource(id);
  if (!source || !source.enabled) return false;
  return source.status === 'fresh' || source.status === 'stale' || source.status === 'very_stale';
}

function computeCoverageAwareEventScore(components: ComponentScores): number {
  let weightedValue = 0;
  let availableWeight = 0;
  let totalWeight = 0;

  (Object.keys(COMPONENT_WEIGHTS) as Array<keyof ComponentScores>).forEach((key) => {
    const weight = COMPONENT_WEIGHTS[key];
    totalWeight += weight;
    const available = COMPONENT_SOURCE_MAP[key].some(isSourceUsable);
    if (!available) return;
...
export function calculateCII(): CountryScore[] {
  ...
  for (const code of countryCodes) {
    ...
    const eventScore = computeCoverageAwareEventScore(components);
