...
export class InsightsPanel extends Panel {
  ...
  private lastDeepIntelUpdate = 0;
  private cachedDeepIntel: DeepIntelReport | null = null;
  ...
  private static readonly DEEP_INTEL_COOLDOWN_MS = 180000; // 3 min cooldown for multi-pass synth
  private static readonly BRIEF_CACHE_KEY = 'summary:world-brief';
  private static readonly DEEP_INTEL_CACHE_KEY = 'summary:world-deep-intel';
  ...
  private async loadDeepIntelFromCache(): Promise<boolean> {
    if (this.cachedDeepIntel) return false;
    const entry = await getPersistentCache<DeepIntelReport>(InsightsPanel.DEEP_INTEL_CACHE_KEY);
    if (!entry?.data?.synthesis || !entry?.data?.risk || !entry?.data?.scenarios || !entry?.data?.contrarian) {
      return false;
    }
    this.cachedDeepIntel = entry.data;
    this.lastDeepIntelUpdate = entry.updatedAt;
    return true;
  }

  private async generateDeepIntel(
    titles: string[],
    geoContext: string,
    summarizeOpts: SummarizeOptions,
    onProgress?: (message: string) => void,
  ): Promise<DeepIntelReport | null> {
    if (titles.length < 2) return null;

    const baseContext = geoContext ? `${geoContext}\n` : '';
    const deepOpts: SummarizeOptions = { ...summarizeOpts, mode: 'deep' };

    const [synthesis, risk, scenarios, contrarian] = await Promise.all([
      generateSummary(
        titles,
        undefined,
        `${baseContext}TASK: Build an integrated executive intelligence synthesis across all provided headlines.`,
        undefined,
        deepOpts,
      ),
      generateSummary(
        titles,
        undefined,
        `${baseContext}TASK: Emphasize escalation pathways, market/security spillovers, and fragile dependencies.`,
        undefined,
        deepOpts,
      ),
      generateSummary(
        titles,
        undefined,
        `${baseContext}TASK: Prioritize scenario modeling. Include trigger conditions and probability confidence.`,
        undefined,
        deepOpts,
      ),
      generateSummary(
        titles,
        undefined,
        `${baseContext}TASK: Act as a red-team analyst. Challenge the dominant narrative and flag overconfidence.`,
        undefined,
        deepOpts,
      ),
    ]);

    const synthesisText = synthesis?.summary?.trim() || '';
    const riskText = risk?.summary?.trim() || '';
    const scenariosText = scenarios?.summary?.trim() || '';
    const contrarianText = contrarian?.summary?.trim() || '';

    const completed = [synthesisText, riskText, scenariosText, contrarianText].filter(Boolean).length;
    onProgress?.(`Deep intel completed ${completed}/4 passes`);
    if (completed < 2) {
      return null;
    }

    return {
      synthesis: synthesisText || 'No synthesis generated.',
      risk: riskText || 'No risk chain generated.',
      scenarios: scenariosText || 'No scenario set generated.',
      contrarian: contrarianText || 'No contrarian view generated.',
    };
  }
  ...
  public async updateInsights(clusters: ClusteredEvent[]): Promise<void> {
    ...
    try {
      ...
      const sentimentTitles = importantClusters.slice(0, 5).map(c => c.primaryTitle);
      const llmTitles = importantClusters.slice(0, 12).map(c => c.primaryTitle);
      ...
      if (mlWorker.isAvailable) {
        sentiments = await mlWorker.classifySentiment(sentimentTitles).catch(() => null);
      }
      ...
      if (!worldBrief || now - this.lastBriefUpdate > InsightsPanel.BRIEF_COOLDOWN_MS) {
        ...
        const result = await generateSummary(llmTitles, (_step, _total, msg) => {
          // Show sub-progress for summarization
          this.setProgress(3, totalSteps, `Generating brief: ${msg}`);
        }, geoContext, undefined, { ...summarizeOpts, mode: 'brief' });
      ...
      // Step 4: Generate deep multi-pass intel
      const loadedDeepFromPersistentCache = await this.loadDeepIntelFromCache();
      if (this.updateGeneration !== thisGeneration) return;

      let deepIntel = this.cachedDeepIntel;
      let usedCachedDeepIntel = loadedDeepFromPersistentCache;
      if (!deepIntel || now - this.lastDeepIntelUpdate > InsightsPanel.DEEP_INTEL_COOLDOWN_MS) {
        this.setProgress(4, totalSteps, 'Generating deep intelligence (multi-pass)...');
        deepIntel = await this.generateDeepIntel(llmTitles, geoContext, summarizeOpts, (msg) => {
          this.setProgress(4, totalSteps, msg);
        });
        if (this.updateGeneration !== thisGeneration) return;

        if (deepIntel) {
          this.cachedDeepIntel = deepIntel;
          this.lastDeepIntelUpdate = now;
          usedCachedDeepIntel = false;
          void setPersistentCache(InsightsPanel.DEEP_INTEL_CACHE_KEY, deepIntel);
        }
      } else {
        usedCachedDeepIntel = true;
        this.setProgress(4, totalSteps, 'Using cached deep intelligence...');
      }

      const hasLlmOutput = Boolean(worldBrief || deepIntel);
      const fullyCached = usedCachedBrief && usedCachedDeepIntel;
      this.setDataBadge(hasLlmOutput ? (fullyCached ? 'cached' : 'live') : 'unavailable');

      // Step 5: Wait for parallel analysis to complete
      this.setProgress(5, totalSteps, 'Multi-perspective analysis...');
      await parallelPromise;

      if (this.updateGeneration !== thisGeneration) return;

      this.renderInsights(importantClusters, sentiments, worldBrief, deepIntel);
    } catch (error) {
  ...
  private renderInsights(
    ...
    deepIntel: DeepIntelReport | null,
  ): void {
    const briefHtml = worldBrief ? this.renderWorldBrief(worldBrief) : '';
    const deepIntelHtml = deepIntel ? this.renderDeepIntel(deepIntel) : '';
  ...
  private renderDeepIntel(report: DeepIntelReport): string {
    return `
      <div class="insights-section insights-deep-intel">
        <div class="insights-section-title">DEEP INTEL</div>
        <div class="deep-intel-block">
          <div class="deep-intel-label">SYNTHESIS</div>
          <div class="deep-intel-text">${escapeHtml(report.synthesis)}</div>
        </div>
        <div class="deep-intel-block">
          <div class="deep-intel-label">RISK CHAINS</div>
          <div class="deep-intel-text">${escapeHtml(report.risk)}</div>
        </div>
        <div class="deep-intel-block">
          <div class="deep-intel-label">SCENARIOS</div>
          <div class="deep-intel-text">${escapeHtml(report.scenarios)}</div>
        </div>
        <div class="deep-intel-block">
          <div class="deep-intel-label">RED TEAM</div>
          <div class="deep-intel-text">${escapeHtml(report.contrarian)}</div>
        </div>
      </div>
    `;
  }
  ...
  private async onAiFlowChanged(): Promise<void> {
    ...
    this.cachedDeepIntel = null;
    this.lastDeepIntelUpdate = 0;
    try {
      await deletePersistentCache(InsightsPanel.BRIEF_CACHE_KEY);
      await deletePersistentCache(InsightsPanel.DEEP_INTEL_CACHE_KEY);
    } catch {
