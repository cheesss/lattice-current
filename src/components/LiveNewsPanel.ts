...
export const OPTIONAL_LIVE_CHANNELS: LiveChannel[] = [
  ...
  { id: 'abc-news', name: 'ABC News', handle: '@ABCNews', fallbackVideoId: 'yMr0neQhu6c' },
  ...
  { id: 'rt', name: 'RT', handle: '', fallbackVideoId: 'u9foWyMSETk' },
  ...
  { id: 'cti-news', name: 'CTI News (Taiwan)', handle: '@ctinews' },
  ...
  { id: 'rudaw', name: 'Rudaw', hlsUrl: 'https://svs.itworkscdn.net/rudawlive/rudawlive.smil/playlist.m3u8', fallbackVideoId: 'xWXpl7azI8k', useFallbackOnly: true },
  // Africa
  ...
  { id: 'tv5monde-info', name: 'TV5 Monde Info', handle: '@TV5MONDEInfo', fallbackVideoId: 'l8PMl7tUDIE' },
  { id: 'nrk1', name: 'NRK1', handle: '@nrk', fallbackVideoId: 'pykpO5kQJ98' },
  { id: 'aljazeera-balkans', name: 'Al Jazeera Balkans', handle: '@AlJazeeraBalkans', fallbackVideoId: 'gCNeDWCI0vo' },
...
if (import.meta.env.DEV) {
  ...
  for (const id of Object.keys(DIRECT_HLS_MAP)) {
    ...
    else if (!ch.fallbackVideoId && !ch.hlsUrl) console.warn(`[LiveNews] Channel '${id}' in DIRECT_HLS_MAP lacks fallbackVideoId`);
  }
}
...
export class LiveNewsPanel extends Panel {
  ...
  constructor() {
    ...
    // Desktop prefers direct HLS channels first to avoid YouTube bot checks.
    const desktopPreferred = isDesktopRuntime()
      ? this.channels.find((channel) => !!this.getDirectHlsUrl(channel.id))
      : null;
    this.activeChannel = desktopPreferred || this.channels[0]!;
  ...
  private getProxiedHlsUrl(channelId: string): string | undefined {
    ...
    const failedAt = this.hlsFailureCooldown.get(channelId);
    if (failedAt && Date.now() - failedAt < this.HLS_COOLDOWN_MS) return undefined;
    return `${getApiBaseUrl()}/api/hls-proxy?url=${encodeURIComponent(entry.url)}`;
  }

  private get embedOrigin(): string {
    if (isDesktopRuntime()) return getApiBaseUrl();
  ...
  private getChannelDisplayName(channel: LiveChannel): string {
    return channel.hlsUrl && !channel.handle ? `${channel.name} ?뵕` : channel.name;
  }
  ...
  private showBotCheckPrompt(): void {
    ...
    // If YouTube is blocking this channel, automatically switch to a direct HLS
    // channel when available so users still get a working live stream.
    if (isDesktopRuntime()) {
      const fallback = this.channels.find((candidate) =>
        candidate.id !== channel.id && !!this.getDirectHlsUrl(candidate.id));
      if (fallback) {
        void this.switchChannel(fallback);
        return;
      }
    }
