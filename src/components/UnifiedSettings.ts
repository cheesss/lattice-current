...
const DESKTOP_RELEASES_URL = 'https://github.com/cheesss/worldmonitor/releases';
const STARTUP_SETUP_KEYS: RuntimeSecretKey[] = [
  'OPENAI_API_KEY',
  'GROQ_API_KEY',
  'OPENROUTER_API_KEY',
  'FRED_API_KEY',
...
export class UnifiedSettings {
  ...
  private panelFilter = '';
  private startupGateActive = false;
  private startupGateResolve: (() => void) | null = null;
  ...
  constructor(config: UnifiedSettingsConfig) {
    ...
    this.escapeHandler = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !this.startupGateActive) this.close();
    };
    ...
    this.overlay.addEventListener('click', (e) => {
      ...
      if (target === this.overlay) {
        if (this.startupGateActive) return;
        this.close();
        return;
      }
      ...
      if (target.closest('[data-startup-continue]')) {
        this.completeStartupGate();
        return;
      }

      if (target.closest('[data-open-runtime-settings]')) {
        if (this.config.isDesktopApp) {
          void invokeTauri<void>('open_settings_window_command').catch((error) => {
            const status = this.overlay.querySelector<HTMLElement>('#us-startup-status');
            if (status) status.textContent = `Failed to open key manager: ${String(error)}`;
          });
        } else {
      ...
      const signupTarget = target.closest<HTMLElement>('[data-signup-url]');
      if (signupTarget?.dataset.signupUrl) {
        const signupUrl = signupTarget.dataset.signupUrl;
        if (!signupUrl) return;
        if (this.config.isDesktopApp) {
          void invokeTauri<void>('open_url', { url: signupUrl }).catch(() => window.open(signupUrl, '_blank'));
        } else {
      ...
      // Open Glint login window (desktop runtime).
      if (target.closest('[data-glint-login]')) {
        void (async () => {
          const status = this.overlay.querySelector<HTMLElement>('#us-glint-login-status');
          try {
            await invokeTauri<void>('open_glint_login');
            if (status) {
              status.textContent = 'Glint login window opened. Sign in there, return to glint.trade, then click Sync Token.';
            }
          } catch (error) {
            if (status) {
              status.textContent = `Failed to open Glint login window: ${String(error)}`;
            }
          }
        })();
        return;
      ...
      // Sync token from logged-in Glint window.
      if (target.closest('[data-glint-sync]')) {
        void (async () => {
          const status = this.overlay.querySelector<HTMLElement>('#us-glint-login-status');
          try {
            ...
            if (token && token.trim()) {
              ...
              status.textContent = 'Failed to sync token: empty token returned.';
            }
          } catch (error) {
            if (status) {
              status.textContent = `Failed to sync token: ${String(error)}`;
            }
          }
        })();
        return;
  ...
  public openStartupGate(): Promise<void> {
    this.startupGateActive = true;
    this.activeTab = 'general';
    this.render();
    this.overlay.classList.add('startup-gate');
    this.overlay.classList.add('active');
    localStorage.setItem('wm-settings-open', '1');
    document.addEventListener('keydown', this.escapeHandler);
    if (this.config.isDesktopApp) {
      void invokeTauri<void>('open_settings_window_command').catch(() => {
        // Keep the startup gate visible even if settings window fails to open.
      });
    }
    return new Promise<void>((resolve) => {
      this.startupGateResolve = resolve;
    });
  }
  ...
  public destroy(): void {
    if (this.startupGateResolve) {
      this.startupGateResolve();
      this.startupGateResolve = null;
    }
    document.removeEventListener('keydown', this.escapeHandler);
    this.overlay.remove();
  }
  ...
  private render(): void {
    ...
    const startupMode = this.startupGateActive;

    this.overlay.innerHTML = `
      <div class="modal unified-settings-modal${startupMode ? ' startup-mode' : ''}">
        <div class="modal-header">
          <span class="modal-title">${startupMode ? 'API & Authentication Setup' : t('header.settings')}</span>
          ${startupMode ? '' : '<button class="modal-close unified-settings-close">×</button>'}
        </div>
        ${startupMode ? '' : `
        ...
        `}
        <div class="unified-settings-tab-panel${startupMode || this.activeTab === 'general' ? ' active' : ''}" data-panel-id="general">
          ${this.renderGeneralContent()}
        </div>
        <div class="unified-settings-tab-panel${!startupMode && this.activeTab === 'panels' ? ' active' : ''}" data-panel-id="panels">
        ...
        <div class="unified-settings-tab-panel${!startupMode && this.activeTab === 'sources' ? ' active' : ''}" data-panel-id="sources">
          <div class="unified-settings-region-wrapper">
  ...
  private renderGeneralContent(): string {
    ...
    const glintConfigured = Boolean(getGlintAuthToken());

    let html = '';
    if (this.startupGateActive) {
      html += this.renderStartupGateContent();
    }

    if (!this.startupGateActive) {
      html += `
        <div class="ai-flow-section-label">Runtime</div>
        <div class="ai-flow-cta">
          <div class="ai-flow-cta-title">API & Authentication</div>
          <div class="ai-flow-cta-desc">Open the full key manager window to enter, review, and test API credentials.</div>
          <button type="button" class="sources-select-all" data-open-runtime-settings>Open API Key Manager</button>
          <div class="ai-flow-cta-desc" id="us-startup-status" style="margin-top:8px;"></div>
        </div>
      `;
    }
    ...
    // Glint token section
    html += `<div class="ai-flow-section-label">Glint Feed</div>`;
    html += `<div class="ai-flow-toggle-row">
      <div class="ai-flow-toggle-label-wrap">
        <div class="ai-flow-toggle-label">Auth Token</div>
        <div class="ai-flow-toggle-desc">Required for private Glint feed and realtime updates.</div>
      </div>
    </div>`;
    html += `
      <div class="ai-flow-cta">
        ...
        <div class="settings-input-wrapper" style="margin-top:8px;">
          <input type="password" id="us-glint-token" placeholder="${glintConfigured ? 'Configured (enter new token to replace)' : 'Paste Glint auth token'}" autocomplete="off" />
        </div>
        <div style="display:flex; gap:8px; margin-top:8px;">
          <button type="button" class="sources-select-all" data-glint-save>Save Token</button>
          <button type="button" class="sources-select-none" data-glint-clear>Clear Token</button>
        </div>
      </div>
    `;
    ...
    if (this.startupGateActive) {
      html += `
        <div class="startup-gate-actions" style="margin-top:12px;">
          <button type="button" class="sources-select-all" data-startup-continue>Continue to Dashboard</button>
        </div>
      `;
    }
  ...
  private renderStartupGateContent(): string {
    const keyRows = STARTUP_SETUP_KEYS.map((key) => {
      const signupUrl = SIGNUP_URLS[key];
      const label = HUMAN_LABELS[key] ?? key;
      const action = signupUrl
        ? `<button type="button" class="startup-key-link" data-signup-url="${escapeHtml(signupUrl)}">Get key</button>`
        : '<span class="startup-key-link disabled">manual</span>';
      return `
        <div class="startup-key-row">
          <span class="startup-key-name">${escapeHtml(label)}</span>
          ${action}
        </div>
      `;
    }).join('');

    const desktopAction = this.config.isDesktopApp
      ? `<button type="button" class="sources-select-all" data-open-runtime-settings>Open API Key Manager</button>`
      : `<a href="${DESKTOP_RELEASES_URL}" target="_blank" rel="noopener noreferrer" class="ai-flow-cta-link">Desktop key manager is available in Tauri build</a>`;

    return `
      <div class="startup-gate-panel">
        <div class="startup-gate-title">Configure keys before loading live feeds</div>
        <div class="startup-gate-desc">1) Open key manager and save required keys. 2) Optional: set Glint token below. 3) Continue to dashboard.</div>
        <div class="startup-gate-actions">
          ${desktopAction}
        </div>
        <div class="startup-key-grid">
          ${keyRows}
        </div>
        <div class="ai-flow-cta-desc" id="us-startup-status" style="margin-top:8px;"></div>
      </div>
  ...
  private completeStartupGate(): void {
    if (!this.startupGateActive) return;
    this.startupGateActive = false;
    this.overlay.classList.remove('startup-gate');
    this.overlay.classList.remove('active');
    localStorage.removeItem('wm-settings-open');
    document.removeEventListener('keydown', this.escapeHandler);
    if (this.startupGateResolve) {
