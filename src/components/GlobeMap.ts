...
export class GlobeMap {
  ...
  private async initGlobe(): Promise<void> {
    ...
    const configuredScale = getGlobeRenderScale();
    const initialScale: GlobeRenderScale = configuredScale === 'auto' ? '1' : configuredScale;
    ...
    const config: ConfigOptions = {
      animateIn: false,
      rendererConfig: {
        // Prefer low-power context on desktop to keep GPU usage/thermals lower.
        powerPreference: desktop ? 'low-power' : 'default',
    ...
    this.unsubscribeGlobeQuality = subscribeGlobeRenderScaleChange((scale) => {
      const effectiveScale: GlobeRenderScale = scale === 'auto' ? '1' : scale;
      this.applyRenderQuality(effectiveScale);
      this.applyPerformanceProfile(resolvePerformanceProfile(effectiveScale));
    });
  ...
  private scoreGlobeMarker(marker: GlobeMarker): number {
    switch (marker._kind) {
      case 'conflict':
      case 'conflictZone':
        return 100;
      case 'iran':
        return marker.severity === 'high' ? 95 : marker.severity === 'critical' ? 100 : 88;
      case 'hotspot':
        return 80 + Math.min(20, Math.round(marker.escalationScore * 4));
      case 'flight':
      case 'vessel':
        return 74;
      case 'newsLocation':
        switch ((marker.threatLevel || '').toLowerCase()) {
      ...
      case 'aisDisruption':
        return 70;
      case 'earthquake':
        return 66 + Math.min(20, Math.round(marker.magnitude * 2));
      default:
        return 50;
    }
  }

  private capGlobeMarkers<T extends GlobeMarker>(items: T[], maxItems: number): T[] {
    if (items.length <= maxItems) return items;
    return [...items]
      .sort((a, b) => this.scoreGlobeMarker(b) - this.scoreGlobeMarker(a))
      .slice(0, maxItems);
  }
  ...
  private flushMarkersImmediate(): void {
    ...
    const pov = this.globe.pointOfView();
    const altitude = Number.isFinite(pov?.altitude) ? pov.altitude : 1;
    const budgetScale = altitude > 1.4 ? 0.45 : altitude > 0.9 ? 0.65 : 1;
    const cap = (count: number, floor = 20): number => Math.max(floor, Math.round(count * budgetScale));
    const pushCapped = <T extends GlobeMarker>(items: T[], maxItems: number, floor = 20): void => {
      markers.push(...this.capGlobeMarkers(items, cap(maxItems, floor)));
    };

    if (this.layers.hotspots) pushCapped(this.hotspots, 180);
    if (this.layers.conflicts) pushCapped(this.conflictZoneMarkers, 80);
    if (this.layers.bases) pushCapped(this.milBaseMarkers, 90);
    if (this.layers.nuclear) pushCapped(this.nuclearSiteMarkers, 70);
    if (this.layers.irradiators) pushCapped(this.irradiatorSiteMarkers, 90);
    if (this.layers.spaceports) pushCapped(this.spaceportSiteMarkers, 80);
    if (this.layers.military) {
      pushCapped(this.flights, 220);
      pushCapped(this.vessels, 220);
    }
    if (this.layers.weather) pushCapped(this.weatherMarkers, 120);
    if (this.layers.natural) {
      pushCapped(this.naturalMarkers, 140);
      pushCapped(this.earthquakeMarkers, 150);
    }
    if (this.layers.economic) pushCapped(this.economicMarkers, 120);
    if (this.layers.datacenters) pushCapped(this.datacenterMarkers, 150);
    if (this.layers.waterways) pushCapped(this.waterwayMarkers, 80);
    if (this.layers.minerals) pushCapped(this.mineralMarkers, 110);
    if (this.layers.flights) pushCapped(this.flightDelayMarkers, 160);
    if (this.layers.ais) pushCapped(this.aisMarkers, 160);
    if (this.layers.iranAttacks) pushCapped(this.iranMarkers, 160);
    if (this.layers.outages) pushCapped(this.outageMarkers, 160);
    if (this.layers.cyberThreats) pushCapped(this.cyberMarkers, 160);
    if (this.layers.fires) pushCapped(this.fireMarkers, 200);
    if (this.layers.protests) pushCapped(this.protestMarkers, 180);
    if (this.layers.ucdpEvents) pushCapped(this.ucdpMarkers, 160);
    if (this.layers.displacement) pushCapped(this.displacementMarkers, 140);
    if (this.layers.climate) pushCapped(this.climateMarkers, 140);
    if (this.layers.gpsJamming) pushCapped(this.gpsJamMarkers, 150);
    if (this.layers.techEvents) pushCapped(this.techMarkers, 140);
    if (this.layers.cables) {
      pushCapped(this.cableAdvisoryMarkers, 130);
      pushCapped(this.repairShipMarkers, 90);
    }
    // Do not always dump all news points on globe; treat as contextual overlay.
    if (this.layers.hotspots || this.layers.conflicts || this.layers.iranAttacks) {
      pushCapped(this.newsLocationMarkers, 240);
    }
    markers.push(...this.flashMarkers);

    const hardCap = cap(1300, 260);
    const finalMarkers = this.capGlobeMarkers(markers, hardCap);

    try {
      this.globe.htmlElementsData(finalMarkers);
  ...
  private initStaticLayers(): void {
    this.rebuildMilitaryBaseMarkers();
  ...
  private rebuildMilitaryBaseMarkers(): void {
    const source = this.runtimeMilitaryBases && this.runtimeMilitaryBases.length > 0
      ? this.runtimeMilitaryBases
      : (MILITARY_BASES as MilitaryBase[]);
    this.milBaseMarkers = source.map((b) => ({
  ...
  private applyRenderQuality(scale?: GlobeRenderScale, width?: number, height?: number): void {
    ...
    try {
      const desktop = isDesktopRuntime();
      const effectiveScale: GlobeRenderScale = (scale ?? getGlobeRenderScale()) === 'auto' ? '1' : (scale ?? getGlobeRenderScale());
      const pr = desktop
        ? 1
        : resolveGlobePixelRatio(effectiveScale);
  ...
  private applyPerformanceProfile(profile: GlobePerformanceProfile): void {
    if (!this.globe || !this.initialized || this.destroyed) return;
    const desktop = isDesktopRuntime();
    const disablePulse = profile.disablePulseAnimations || desktop;
    const disableDash = profile.disableDashAnimations || desktop;
    const disableAtmosphere = profile.disableAtmosphere || desktop;

    const prevPulse = this._pulseEnabled;
    this._pulseEnabled = !disablePulse;
