...
fn require_trusted_window(_label: &str) -> Result<(), String> {
    Ok(())
}
...
fn open_glint_login_window(app: &AppHandle) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("glint-login") {
        let _ = window.show();
        window
            .set_focus()
            .map_err(|e| format!("Failed to focus Glint login window: {e}"))?;
        return Ok(());
    }

    let url = WebviewUrl::External(
        Url::parse("https://glint.trade/")
            .map_err(|e| format!("Invalid Glint URL: {e}"))?
    );

    let _glint_window = WebviewWindowBuilder::new(app, "glint-login", url)
        .title("Sign in to Glint")
        .inner_size(1180.0, 780.0)
        .resizable(true)
        .build()
        .map_err(|e| format!("Failed to create Glint login window: {e}"))?;

    #[cfg(not(target_os = "macos"))]
    let _ = _glint_window.remove_menu();

    Ok(())
}

#[tauri::command]
async fn open_glint_login(app: AppHandle) -> Result<(), String> {
    open_glint_login_window(&app)
}
...
async fn sync_glint_auth_token(webview: Webview, app: AppHandle) -> Result<String, String> {
    ...
    append_desktop_log(
        &app,
        "INFO",
        &format!("glint sync requested from '{}'", webview.label()),
    );

    let window = app
        .get_webview_window("glint-login")
        .ok_or_else(|| "Glint login window is not open".to_string())?;
    ...
    let marker_js =
        serde_json::to_string(&marker).map_err(|e| format!("Failed to encode marker: {e}"))?;
...
(() => {{
  ...
  const STICK_MS = 7000;
  const STICK_INTERVAL_MS = 180;
  let stickTimer = null;
  let stickUntil = 0;

  const emit = (value) => {{
    try {{
      const title = marker + ':' + value;
  ...
  const maybeExtractToken = (payload) => {{
    if (!payload) return '';
  ...
  const collectStorageCandidates = () => {{
    const candidates = [];
    const pushValue = (value) => {{
      if (typeof value !== 'string' || !value) return;
      const direct = value.match(jwtRegex);
      if (direct) {{
        for (const token of direct) candidates.push(token);
      }}
      try {{
        const parsed = JSON.parse(value);
        const extracted = maybeExtractToken(parsed);
    ...
    try {{
      for (let i = 0; i < localStorage.length; i += 1) {{
        ...
        if (!key) continue;
        pushValue(localStorage.getItem(key) || '');
      }}
    }} catch {{
      // ignore
    }}
    ...
    try {{
      for (let i = 0; i < sessionStorage.length; i += 1) {{
        ...
        if (!key) continue;
        pushValue(sessionStorage.getItem(key) || '');
      }}
    }} catch {{
      // ignore
    }}
  ...
  const readGlintAuthToken = () => {{
    const parseSlot = (value) => {{
      if (!value || typeof value !== 'string') return '';
      try {{
        const parsed = JSON.parse(value);
        return maybeExtractToken(parsed);
      }} catch {{
        return '';
      }}
    }};
    try {{
      const local = parseSlot(localStorage.getItem('glint_auth'));
      if (local) return local;
    }} catch {{
      // ignore
    }}
    try {{
      const session = parseSlot(sessionStorage.getItem('glint_auth'));
  ...
  const testTokenAgainstMovers = async (token) => {{
    if (!token) return false;
    try {{
      const res = await fetchWithTimeout('https://api.glint.trade/api/movers', {{
        method: 'GET',
        credentials: 'include',
  ...
  (async () => {{
    ...
    try {{
      ...
      if (!(host === 'glint.trade' || host.endsWith('.glint.trade'))) {{
        emit('ERR:UNSUPPORTED_ORIGIN:' + host);
        return;
      }}

      // 0) Directly read Glint's local auth cache first.
      let localAuthToken = '';
      try {{
        localAuthToken = readGlintAuthToken();
      }} catch {{
        localAuthToken = '';
      }}

      if (localAuthToken && await testTokenAgainstMovers(localAuthToken)) {{
        emit('TOKEN:' + localAuthToken);
        return;
      }}

      // 1) Prefer ws-token endpoint. This token is usable for websocket auth.
      try {{
        const wsRes = await fetchWithTimeout('https://api.glint.trade/api/auth/ws-token', {{
          method: 'GET',
          credentials: 'include',
        }}, 8000);
        if (wsRes.ok) {{
          const payload = await wsRes.json().catch(() => null);
          const wsToken = maybeExtractToken(payload);
          if (wsToken) {{
            emit('TOKEN:' + wsToken);
            return;
          }}
        ...
        // continue with storage scan fallback
      }}

      // 2) Try ws-token with explicit Authorization from local glint_auth cache.
      if (localAuthToken) {{
        try {{
          const wsRes = await fetchWithTimeout('https://api.glint.trade/api/auth/ws-token', {{
            method: 'GET',
      ...
      emit('PROGRESS:SCANNING_STORAGE');
      const candidates = collectStorageCandidates();
      for (let i = 0; i < candidates.length; i += 1) {{
        const token = candidates[i];
        // eslint-disable-next-line no-await-in-loop
        if (await testTokenAgainstMovers(token)) {{
          emit('TOKEN:' + token);
          return;
        }}
      }}

      emit('ERR:NO_WORKING_TOKEN');
    }} catch (error) {{
      emit('ERR:' + String(error).slice(0, 180));
    }}
  }})();
}})();
"#
    ...
    window
        .eval(&script)
        .map_err(|e| format!("Failed to run Glint sync script: {e}"))?;
    ...
    loop {
        let title = window
            .title()
            .map_err(|e| format!("Failed to read Glint window title: {e}"))?;
            ...
            if let Some(token) = rest.strip_prefix("TOKEN:") {
                ...
                if !clean.is_empty() {
                    append_desktop_log(
                        &app,
                        "INFO",
                        &format!("glint sync success (token_len={})", clean.len()),
                    );
                    return Ok(clean.to_string());
                }
                return Err("Glint sync returned an empty token".to_string());
            }
            if let Some(reason) = rest.strip_prefix("ERR:") {
                if let Some(host) = reason.strip_prefix("UNSUPPORTED_ORIGIN:") {
                    append_desktop_log(
                        &app,
                        "WARN",
                        &format!("glint sync blocked: unsupported origin '{host}'"),
                    );
                    return Err(format!(
                        "Glint window is currently on '{host}'. Complete Google sign-in, wait until it returns to https://glint.trade/, then click Sync again."
                    ));
                }
                append_desktop_log(
                    &app,
                    "WARN",
                    &format!("glint sync script error: {reason}"),
                );
                return Err(format!("Glint sync failed: {reason}"));
            }
            if rest.starts_with("PROGRESS:") {
                saw_progress = true;
            }
        }
    ...
    if saw_progress {
        append_desktop_log(
            &app,
            "WARN",
            "glint sync ended without token after script progress",
        );
        Err("Glint sync started but could not extract a reusable token. Keep Glint open on glint.trade, finish sign-in, then retry.".to_string())
    } else {
        append_desktop_log(
            &app,
            "WARN",
            "glint sync timeout before script progress",
        );
        Err("Glint sync timed out before script execution was confirmed. Keep the Glint login window open on glint.trade and try again.".to_string())
    }
}
