...
test('accepts OPENAI_API_KEY via /api/local-env-update', async () => {
  const localApi = await setupApiDir({});
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/local-env-update`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ key: 'OPENAI_API_KEY', value: 'sk-test-openai-key' }),
    });
    assert.equal(response.status, 200);
    const body = await response.json();
    assert.equal(body.ok, true);
    assert.equal(body.key, 'OPENAI_API_KEY');
    assert.equal(process.env.OPENAI_API_KEY, 'sk-test-openai-key');
  } finally {
...
test('reports Codex login status via /api/local-codex-status', async () => {
  const localApi = await setupApiDir({});
  __setCodexRunnerForTests(async (args) => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/local-codex-status`);
    assert.equal(response.status, 200);
    const body = await response.json();
    assert.equal(body.available, true);
    assert.equal(body.loggedIn, true);
    assert.equal(body.message, 'Codex CLI logged in');
  } finally {
    __setCodexRunnerForTests(null);
    await app.close();
    await localApi.cleanup();
  }
});

test('summarizes with Codex CLI via /api/local-codex-summarize', async () => {
  const localApi = await setupApiDir({});
  let execSeen = false;
  __setCodexRunnerForTests(async (args) => {
    ...
    if (args[0] === 'exec') {
      ...
      return {
        code: 0,
        stdout: '{"type":"item.completed","item":{"type":"agent_message","text":"This is the summary."}}\n',
        stderr: '',
        timedOut: false,
      };
    }
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/local-codex-summarize`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        mode: 'brief',
    ...
    assert.equal(body.summary, 'This is the summary.');
    assert.equal(execSeen, true);
  } finally {
    __setCodexRunnerForTests(null);
    await app.close();
    await localApi.cleanup();
...
test('validates OPENAI_API_KEY via /api/local-validate-secret', async () => {
  const localApi = await setupApiDir({});
