...
const pythonScript = path.join(scriptDir, 'compute_abnormal_returns.py');
const legacyScript = path.join(scriptDir, 'compute-abnormal-returns.legacy.mjs');

function parseArgs(argv) {
  const passthrough = [];
  let engine = String(process.env.LATTICE_ABNORMAL_RETURNS_ENGINE || 'auto').trim().toLowerCase();
...
function tryRunPython(args, allowFallback) {
  ...
  for (const candidate of candidates) {
    ...
    if (result.error) {
      console.error(`[abnormal-returns] failed to start ${command}: ${result.error.message}`);
    ...
    console.warn('[abnormal-returns] Python engine exited non-zero, falling back to legacy JS engine.');
    return false;
  }

  if (!allowFallback) {
    console.error('[abnormal-returns] no Python interpreter found. Set LATTICE_PYTHON or install python.');
    process.exit(1);
  }
  return false;
}
...
function runLegacy(args) {
  ...
  if (result.error) {
    console.error(`[abnormal-returns] legacy JS engine failed: ${result.error.message}`);
    process.exit(1);
  }
  process.exit(result.status ?? 1);
}
