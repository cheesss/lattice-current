...
async function runCodexAnalysis(state) {
  ...
  const prompt = [
    ...
    '',
    'Your job is to identify important uncovered global risks and remove redundant collection queries.',
    'You are not writing a narrative memo. You are making a concrete collection-plan decision.',
    '',
    ...
    '',
    'ANALYSIS STEPS:',
    '1. Identify important risks that are missing from the current query set.',
    '2. Check whether each proposed risk is truly different from the existing queries.',
    '3. Suggest a narrow GDELT query for each valid new risk.',
    '4. Suggest only symbols that clearly map to the risk.',
    '5. Flag current queries only when they are obviously redundant or low-signal.',
    '',
    ...
    '',
    'RULES:',
    '- Do not propose vague or duplicate issues.',
    '- Do not propose out-of-scope issues.',
    '- Use only symbols you can justify from the supplied risks.',
    '- If nothing should be removed, return an empty removeQueries array.',
    '',
    'JSON schema:',
    '{',
