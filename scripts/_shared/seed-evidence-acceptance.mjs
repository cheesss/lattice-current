...
function acceptanceUseFor(evidenceClass = '', row = {}) {
  ...
  if (evidenceClass === 'holdout_validation') return 'supporting_context';
...
function sourceQueryEvidenceIsDirect(row = {}, evidenceClass = '') {
  ...
  if (evidenceClass === 'negative_control') {
    return use === 'negative_control_candidate'
      && /invalidator|checked_no_direct|supported_constraint|no direct invalidator|no direct contradiction/i.test(compact([
...
export function evaluateSeedEvidenceAcceptance(row = {}, {
  ...
  if (String(row.acceptanceVerdict || '').includes('no_accepted_result')) blockers.push('raw_result_not_accepted_by_collector');
  if (compact(row.evidenceUse || row.evidence_use) === 'weak_noise') blockers.push('weak_evidence_not_accepted');
  ...
  if (isSeedBiasSourceQuery(row) && evidenceClass === 'holdout_validation' && row.holdoutConfirmed !== true) {
    blockers.push('holdout_validation_requires_official_independent_confirmation');
  }
  ...
  for (const row of asArray(rawRows)) {
    ...
    if (verdict.accepted) {
      acceptedEvidence.push({
        ...
        promotionEligible: verdict.promotionEligible,
        coveredEvidenceClasses: verdict.coveredEvidenceClasses,
        validationFixtureOnly: row.validationFixtureOnly === true,
        fixtureBackedProviderExecution: row.fixtureBackedProviderExecution === true,
