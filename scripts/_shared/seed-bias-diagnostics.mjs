...
export const NEGATIVE_CONTROL_SURVIVAL_STATUSES = Object.freeze({
  ...
  CHECKED_NO_DIRECT_LIMITED_SCOPE: 'CHECKED_NO_DIRECT_LIMITED_SCOPE',
  CHECKED_NO_DIRECT_SUFFICIENT_SCOPE: 'CHECKED_NO_DIRECT_SUFFICIENT_SCOPE',
  WEAKENED: 'WEAKENED',
  REJECTED: 'REJECTED',
...
function negativeSurvivalRate(negativeControls = {}) {
  ...
  if (!rows.length) return 0;
  const survived = rows.filter((row) => ['SURVIVED', 'CHECKED_NO_DIRECT', 'CHECKED_NO_DIRECT_SUFFICIENT_SCOPE'].includes(row.survivalStatus)).length;
...
export function evaluateNegativeControlSurvival(seeds = [], evidence = [], options = {}) {
  ...
  const items = rows.map((seed) => {
    ...
    const sufficientScoped = matched.filter((item) => /CHECKED_NO_DIRECT_SUFFICIENT_SCOPE|checked_no_direct_sufficient_scope/i.test(JSON.stringify(item)));
    const limitedScoped = matched.filter((item) => /CHECKED_NO_DIRECT_LIMITED_SCOPE|checked_no_direct_limited_scope/i.test(JSON.stringify(item)));
    ...
    } else if (sufficientScoped.length) {
      survivalStatus = NEGATIVE_CONTROL_SURVIVAL_STATUSES.CHECKED_NO_DIRECT_SUFFICIENT_SCOPE;
      survivalReason = 'negative-control search found no direct invalidator with sufficient scope';
    } else if (limitedScoped.length) {
      survivalStatus = NEGATIVE_CONTROL_SURVIVAL_STATUSES.CHECKED_NO_DIRECT_LIMITED_SCOPE;
      survivalReason = 'negative-control search found no direct invalidator, but scope remains limited';
...
export function evaluateAutonomousSeedReportCandidateGate(rowOrSeed = {}, context = {}) {
  ...
  if (seed.scores?.parentOnlyDueToKnownNarrative === true || seed.metadata?.parentOnlyDueToKnownNarrative === true || seed.metadata?.userCrossThemePrior?.parentOnlyDueToKnownNarrative === true) {
    blockers.push('known_narrative_parent_requires_child_decomposition');
  }
  ...
  const negativeClosed = ['SURVIVED', 'CHECKED_NO_DIRECT', 'CHECKED_NO_DIRECT_SUFFICIENT_SCOPE'].includes(negative.survivalStatus);
