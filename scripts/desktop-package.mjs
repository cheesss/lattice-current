...
const noBundle = hasFlag('no-bundle');
...
if (showHelp) {
  console.log('Usage: npm run desktop:package -- --os <macos|windows|linux> --variant <full|tech|finance> [--sign] [--skip-node-runtime] [--no-bundle]');
  process.exit(0);
}

if (!validOs.has(os)) {
  console.error('Usage: npm run desktop:package -- --os <macos|windows|linux> --variant <full|tech|finance> [--sign] [--skip-node-runtime] [--no-bundle]');
  process.exit(1);
}
...
// Some enterprise WDAC/App Control policies block cargo build artifacts under
// user profile paths. Use a stable root target dir on Windows unless caller overrides.
if (os === 'windows' && !env.CARGO_TARGET_DIR) {
  env.CARGO_TARGET_DIR = 'C:\\wm-cargo-target';
}
const cliArgs = ['build'];
...
if (noBundle) {
  ...
  cliArgs.push('--bundles', bundles);
}

console.log(
  `[desktop-package] OS=${os} VARIANT=${variant} BUNDLES=${noBundle ? 'none(--no-bundle)' : bundles} SIGN=${sign ? 'on' : 'off'}`
);
