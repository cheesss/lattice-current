...
const MAPPINGS = {
  ...
  'robotics-automation': ['ROBO', 'ARKQ', 'NVDA', 'ROK', 'ABB'],
  ...
  'optical-computing': ['IIVI', 'COHR', 'LITE', 'NVDA', 'AMD'],
...
async function main() {
  ...
  try {
    ...
    let inserted = 0;
    let skipped = 0;
    for (const [theme, symbols] of Object.entries(MAPPINGS)) {
      for (let i = 0; i < symbols.length; i += 1) {
        const symbol = symbols[i];
        // tier the quality score so first symbol scores slightly higher than later ones
        const qScore = (DEFAULT_SCORES.quality_score - i * 0.02).toFixed(4);
        ...
        `, [
          theme, symbol,
          DEFAULT_SCORES.avg_abs_reaction, DEFAULT_SCORES.reaction_count, DEFAULT_SCORES.baseline_avg_abs,
          DEFAULT_SCORES.correlation, DEFAULT_SCORES.event_avg_return, DEFAULT_SCORES.baseline_avg_return,
          DEFAULT_SCORES.event_hit_rate, DEFAULT_SCORES.baseline_hit_rate, DEFAULT_SCORES.specificity_score,
          DEFAULT_SCORES.directional_edge, DEFAULT_SCORES.return_shift, DEFAULT_SCORES.theme_coverage_count,
          DEFAULT_SCORES.generic_penalty, DEFAULT_SCORES.outcome_count, DEFAULT_SCORES.outcome_hit_rate,
          DEFAULT_SCORES.outcome_avg_return, qScore,
        ]);
