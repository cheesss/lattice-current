...
export function getCacheKey(
  ...
  const normalizedMode = typeof mode === 'string' ? mode.toLowerCase() : 'brief';
  const headlineWindow = normalizedMode === 'deep' ? 20 : 5;
  const sorted = headlines.slice(0, headlineWindow).sort().join('|');
  const geoHash = geoContext ? ':g' + hashString(geoContext).slice(0, 6) : '';
  const hash = hashString(`${normalizedMode}:${sorted}`);
  ...
  if (normalizedMode === 'translate') {
    ...
    return `summary:${CACHE_VERSION}:${normalizedMode}:${targetLang}:${hash}${geoHash}`;
  }

  return `summary:${CACHE_VERSION}:${normalizedMode}:${normalizedVariant}:${normalizedLang}:${hash}${geoHash}`;
}
...
Produce a dense multi-headline technology intelligence brief using ALL relevant headlines.
Rules:
- Headlines may be unrelated; never merge factual details from different stories into one claim.
- You MAY synthesize cross-headline themes when explicitly labeled as a pattern.
- Avoid filler and narrative padding.
- Prefer concrete entities (company/product/sector/region) and explicit causal links.
- Output format (plain text, exact sections):
Executive Brief:
- 3 bullets
Critical Drivers:
- 3 bullets
...
Watchlist:
- up to 6 bullet items (entities, geographies, assets, or indicators)${langInstruction}`;
      userPrompt = `Build a high-density tech intelligence brief from these headlines:\n${headlineText}${intelSection}`;
    } else {
      systemPrompt = `${dateContext}

Produce a dense multi-headline geopolitical intelligence brief using ALL relevant headlines.
Rules:
- Headlines may be unrelated; never merge factual details from different stories into one claim.
- You MAY synthesize cross-headline patterns if you clearly mark them as inference.
- No fluff, no TV-style lead-ins, no meta-commentary.
- Emphasize actors, locations, escalation signals, and potential market/security spillovers.
- Output format (plain text, exact sections):
Executive Brief:
- 3 bullets
Critical Drivers:
...
Watchlist:
- up to 6 bullet items (entities, geographies, assets, or indicators)${langInstruction}`;
      userPrompt = `Build a high-density geopolitical intelligence brief from these headlines:\n${headlineText}${intelSection}`;
    }
