...
export async function summarizeArticle(
  ...
  const MAX_HEADLINES = isDeepMode ? 40 : 10;
  const MAX_HEADLINE_LEN = 500;
  const MAX_GEO_CONTEXT_LEN = isDeepMode ? 5000 : 2000;
  ...
  try {
    ...
    const { data: result, source } = await cachedFetchJsonWithMeta<{ summary: string; model: string; tokens: number }>(
      ...
      CACHE_TTL_SECONDS,
      async () => {
        const dedupeWindow = isDeepMode ? 20 : 5;
        const uniqueHeadlines = deduplicateHeadlines(headlines.slice(0, dedupeWindow));
        ...
        const maxTokens = normalizedMode === 'deep'
          ? 1400
          : normalizedMode === 'translate'
            ? 220
            : 140;
        const temperature = normalizedMode === 'deep' ? 0.35 : 0.3;
        const topP = normalizedMode === 'deep' ? 0.95 : 0.9;
